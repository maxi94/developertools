...
function toImageItems(rawInput: string): {
  ...
  try {
    ...
    const items = entries.map((entry, index) => {
      const mimeType = entry.mimeType?.startsWith('image/') ? entry.mimeType : 'image/png'
      const blob = base64ToBlob(entry.value, mimeType)
...
export function Base64ImageTool() {
  ...
  const result = useMemo(() => toImageItems(input), [input])
  ...
  return (
    <section className="rounded-3xl border border-slate-300/70 bg-white/80 p-4 shadow-lg shadow-slate-900/10 backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/75 dark:shadow-black/40">
      ...
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
        Soporta una imagen o multiples. Formatos: una cadena por linea, data URL o array JSON.
      </p>
      ...
      <textarea
        ...
        spellCheck={false}
        placeholder='Ejemplo: ["data:image/png;base64,...", "..."]'
      />
