  ...
  const output = useMemo(() => {
    ...
    try {
      return {
        ...
        value: mode === 'encode' ? encodeUrl(input) : decodeUrl(input),
      }
    } catch {
      return { status: 'error' as const, value: 'Entrada invalida para codificacion URL.' }
    }
  }, [input, mode])
    ...
    <section className="rounded-3xl border border-slate-300/70 bg-white/80 p-4 shadow-lg shadow-slate-900/10 backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/75 dark:shadow-black/40">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        ...
        <label className="grid gap-2">
          ...
          <textarea
            className="min-h-[240px] w-full resize-y rounded-2xl border border-slate-300 bg-slate-50 p-3 font-mono text-sm outline-none transition data-[status=error]:border-rose-500 data-[status=error]:text-rose-600 dark:border-slate-600 dark:bg-slate-900/70 dark:text-slate-100 dark:data-[status=error]:text-rose-300"
            value={output.value}
            readOnly
            spellCheck={false}
